...
class TOF(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        if sum(outliers_bool) > (len(outliers_bool) / 2.):
    ...
    def _compute_cutoff(self, cutoff_n):
        ...
        :rtype: float
        """
        if type(cutoff_n) == int or type(cutoff_n) == np.float64 \
                or type(cutoff_n) == float or type(cutoff_n) == np.int64:
            if self.k is None:
                k = self.d_ + 1
            else:
                k = self.k
    ...
    def _compute_p_value(self, outlier_score):
        p_value = (np.argsort(np.argsort(outlier_score)) + 1) / float(len(outlier_score))
        return p_value
    ...
    def _compute_outlier_score(self, X, k=None):
        ...
        outlier_score_ = 1. / self._compute_tof(nearest_indicis, indicis)
    ...
    def _compute_tof(self, nearest_indicis, indicis):
        ...
        """
        q = self.q
        tof = self.centrality_func(np.abs(nearest_indicis - indicis) ** q, axis=1) ** (1. / q)
    ...
    def _compute_perc_cutoff(self, cutoff_n):
        ...
        :rtype: float
        """
        cutoff_len = int(self.X_.shape[0] * (cutoff_n / 100.))
